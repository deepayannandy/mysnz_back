...
router.get("/Dashboard/:sid", async (req, res) => {
  ...
  let addedUserIds = [];
  try {
    const allTransactionToday = await customerHistoryModel.find({
      date: {
        $gt: startDate,
        $lt: endDate,
      },
    ...
    const filteredTransactions_old = allTransactionToday.filter(
      (transactions) => {
        return !transactions.description.includes("Add Old Credit");
      }
    );
    const filteredTransactions = filteredTransactions_old.filter(
      (transactions) => {
        return transactions.storeId == req.params.sid;
      }
    );
    const creditUserList = filteredTransactions.filter((transactions) => {
      return transactions.due > 0;
    });
    console.log(filteredTransactions);
    let finalCreditUserList = [];
    for (let index in creditUserList) {
      const cData = await customerModel.findById(
        creditUserList[index].customerId
      );
    ...
    for (let index in filteredTransactions) {
      ...
      if (
        filteredTransactions[index].description.includes("undefined") ||
        filteredTransactions[index].description.includes("CASH")
      ) {
  ...
  router.get("/signOffReport/:uid", async (req, res) => {
    ...
    for (let index in allTransactionToday) {
      ...
      if (
        ...
        allTransactionToday[index].description === "Meal Order "
      ) {
        cash = cash + allTransactionToday[index].paid;
      }
      if (
        allTransactionToday[index].description.toLowerCase().includes("upi")
      ) {
        upi = upi + allTransactionToday[index].paid;
      }
      if (
        allTransactionToday[index].description.toLowerCase().includes("card")
      ) {
        card = card + allTransactionToday[index].paid;
      }
      due = due + allTransactionToday[index].due;
      if (
        allTransactionToday[index].description
          .toLowerCase()
          .includes("pay dues")
      ) {
