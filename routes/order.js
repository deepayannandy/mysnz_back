...
router.post("/", verify_token, async (req, res) => {
  ...
  for (let i in req.body.orderItems) {
    let message = await updateProductCount(
      ...
      req.body.orderItems[i].qnt
    );
    if (message != "ok") return res.status(500).json({ message: message });
  }
  const transId = `${selectedStore.storeName
    .replace(" ", "")
    .substring(0, 3)
  ...
  if (customer.customerId == undefined) {
    console.log("need to create customer");
  ...
  const newOrderHistory = new orderHistoryModel({
    ...
    status:
      req.body.netPay - req.body.cashIn > 0
        ? req.body.cashIn == ""
          ? "Due"
          : "Partially Paid"
        : "Paid",
  ...
  const newHistory = new historyModel({
    ...
    credit:
      req.body.netPay - req.body.cashIn < 0
        ? 0
        : req.body.netPay - req.body.cashIn,
  ...
  const newCustomerHistory = new customerHistoryModel({
    ...
    netPay: req.body.netPay,
    paid:
      req.body.netPay - req.body.cashIn < 0 ? req.body.netPay : req.body.cashIn,
    due:
      req.body.netPay - req.body.cashIn < 0
        ? 0
        : req.body.netPay - req.body.cashIn,
  ...
  try {
    ...
    res
      .status(201)
      .json({ _id: createdOrderHistory.id, transactionId: transId });
  } catch (error) {
...
router.get("/", verify_token, async (req, res) => {
  ...
  try {
    const mealHistory = await orderHistoryModel.find({
      storeId: loggedInUser.storeId,
    });
