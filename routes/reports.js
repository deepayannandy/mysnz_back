...
router.get("/transactionReport/:storeId/",verify_token, async(req,res)=>{
    ...
    if(req.query.startDate==undefined || req.query.endDate==undefined){ 
        ...
        console.log(today,startDate,endDate)
         allTransactions= await customerHistoryModel.find({$and:
            [{date:{
               $gt: startDate,
               $lt: endDate
           }},{
    ...
    for (let i in allTransactions){
        ...
        netAmount=netAmount+(allTransactions[i].netPay>0?allTransactions[i].netPay:0)
        dues=dues+allTransactions[i].due??0
        discount=discount+(allTransactions[i].discount>0?allTransactions[i].discount:0)
        if(allTransactions[i].description.toLowerCase().includes("pay dues")){
            dues=dues-allTransactions[i].paid??0
        }
        if(allTransactions[i].description.toLowerCase().includes("cash") || allTransactions[i].description.toLowerCase().includes("meal order")){
