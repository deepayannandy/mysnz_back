...
router.post("/", verify_token, async (req, res) => {
  ...
  const newTable = new tableModel({
    ...
    slotWiseMinuteRules: req.body.slotWiseMinuteRules,
...
router.get("/", verify_token, async (req, res) => {
  ...
  try {
    const storeHistory = await tableModel.find({
      storeId: loggedInUser.storeId,
    });
...
router.patch("/:tableId", verify_token, async (req, res) => {
  ...
  if (table.isOccupied)
    return res.status(409).json({ message: "Table is occupied!" });
  ...
  if (req.body.slotWiseMinuteRules != null) {
    table.slotWiseMinuteRules = req.body.slotWiseMinuteRules;
  }
...
router.patch("/switchTable/switch", async (req, res) => {
  ...
  if (!oldTable)
    return res.status(404).json({ message: "From Table not found" });
  ...
  if (!newTable.gameTypes.includes(oldTable.gameData.gameType))
    return res.status(404).json({
      message: `This game type is not available on ${newTable.tableName}`,
    });
  if (newTable.isOccupied)
    return res
      .status(404)
      .json({ message: `${newTable.tableName} is already occupied` });
  if (!oldTable.tableType) {
    return res.status(404).json({
      message: `Please setup a table type for ${newTable.tableName} `,
    });
  } else {
    if (oldTable.tableType != newTable.tableType)
      return res.status(404).json({
        message: `${newTable.tableName} is not a ${oldTable.tableType} Table`,
      });
  }
