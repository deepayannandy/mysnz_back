...
router.post('/login',async (req,res)=>{
    ...
    if(user.userDesignation!="SuperAdmin") return res.status(401).send({"message":"This user is not a SuperAdmin!"});
...
router.get('/whoAmI', verify_token, async (req,res)=>{
    ...
    try{
        ...
        const subscription=  loggedInUser.userDesignation=="SuperAdmin" ? null: activeSubscription[0].subscriptionName?? null
